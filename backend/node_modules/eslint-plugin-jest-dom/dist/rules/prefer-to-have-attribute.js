...
const create = context => ({
  ...
  [`CallExpression[callee.property.name='getAttribute'][parent.callee.name='expect'][parent.parent.property.name=/toBe$|to(Strict)?Equal/]`](node) {
    ...
    context.report({
      ...
      message: `Use toHaveAttribute instead of asserting on getAttribute`,
      fix: fixer => {
        const lastFixer = isNullOrEmpty ? fixer.replaceText(node.parent.parent.parent.arguments[0], sourceCode.getText(node.arguments[0])) : fixer.insertTextBefore(node.parent.parent.parent.arguments[0], `${sourceCode.getText(node.arguments[0])}, `);
        return [fixer.removeRange([node.callee.object.range[1], node.range[1]]), fixer.replaceText(node.parent.parent.property, `${isNullOrEmpty ? "not." : ""}toHaveAttribute`), lastFixer];
      }
    });
  },
