...
const rule = utils_1.ruleCreator({
    ...
    create: (context) => {
        const { couldBeObservable, couldBeSubscription, getType } = eslint_etc_1.getTypeServices(context);
        ...
        function isObservableOrSubscription(node, action) {
            if (!eslint_etc_1.isMemberExpression(node.callee)) {
                return;
            }
        ...
        return {
            ...
            ThisExpression: (node) => {
                let parent = eslint_etc_1.getParent(node);
                while (parent) {
