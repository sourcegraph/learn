...
const rule = utils_1.ruleCreator({
    ...
    create: (context, unused) => {
        ...
        const { couldBeSubject, couldBeType } = eslint_etc_1.getTypeServices(context);
        ...
        return {
            [`ClassProperty[accessibility!=${accessibilityRexExp}]`]: (node) => {
                if (isSubject(node)) {
                    const { key } = node;
                    if (eslint_etc_1.isIdentifier(key)) {
                        context.report({
                            messageId,
                            node: key,
                            data: {
