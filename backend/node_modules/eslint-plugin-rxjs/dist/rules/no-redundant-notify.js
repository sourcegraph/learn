    ...
    create: (context) => {
        ...
        return {
            "ExpressionStatement[expression.callee.property.name=/^(complete|error)$/] + ExpressionStatement[expression.callee.property.name=/^(next|complete|error)$/]": (node) => {
                ...
                if (!eslint_etc_1.isBlockStatement(parent) && !eslint_etc_1.isProgram(parent)) {
                    return;
                }
                const { body } = parent;
                ...
                if (eslint_etc_1.isCallExpression(expression)) {
                    const { callee } = expression;
                    if (eslint_etc_1.isMemberExpression(callee)) {
                        const { property } = callee;
                        if (eslint_etc_1.isIdentifier(property)) {
                            context.report({
                                messageId: "forbidden",
                                node: property,
                            });
...
function getExpressionText(expressionStatement, sourceCode) {
    if (!eslint_etc_1.isCallExpression(expressionStatement.expression)) {
        return undefined;
    }
